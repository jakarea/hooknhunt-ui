      ...
      >
        ...
        <div className="bg-gradient-to-r from-[#ec3137] to-[#8a0f12] text-white px-5 py-4 flex items-center justify-between flex-shrink-0 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold tracking-tight">{t('cart.sidebar.title')}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {cartItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-16 px-4">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t('cart.sidebar.empty')}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 max-w-xs">{t('cart.sidebar.emptyMessage')}</p>
              <button onClick={closeCart} className="px-6 py-3 bg-[#ec3137] hover:bg-[#8a0f12] text-white text-sm font-semibold transition-colors rounded-xl shadow-md hover:shadow-lg">
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                  <div className="flex items-start justify-between gap-2 flex-1">
                    <Link href={`/products/${item.product.slug}`} onClick={closeCart} className="min-w-0 flex-1">
                      <h3 className="font-semibold text-gray-900 dark:text-white text-sm line-clamp-2 hover:text-[#ec3137] transition-colors leading-snug">
        ...
        {cartItems.length > 0 && crossSellProducts.length > 0 && (
          <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-800 px-4 py-3 bg-white dark:bg-[#0a0a0a]">
            <h4 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-3">{t('cart.sidebar.youMightAlsoLike')}</h4>
              ...
              {crossSellProducts.filter(p => !cartItems.some(item => item.product.id === p.id)).slice(0, 2).map((product) => {
                ...
                return (
                  <Link key={product.id} href={`/products/${product.slug}`} onClick={closeCart} className="group border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden hover:border-[#ec3137] transition-all hover:shadow-md">
                    ...
                    <div className="p-2.5">
                      <h5 className="text-xs font-semibold text-gray-800 dark:text-gray-200 line-clamp-2 leading-snug group-hover:text-[#ec3137] transition-colors min-h-[2.25rem]">{product.title}</h5>
                      ...
                      <button
                        ...
                        className="mt-2 w-full py-2 bg-[#ec3137] hover:bg-[#8a0f12] text-white text-xs font-bold rounded-lg transition-colors shadow-sm"
        ...
        {cartItems.length > 0 && (
          <div className="border-t border-gray-200 dark:border-gray-800 px-5 py-4 space-y-3 bg-gray-50 dark:bg-[#0f0f0f] flex-shrink-0">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 text-center leading-relaxed">{t('cart.sidebar.shippingNote')}</p>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={handleViewCart} className="py-2.5 bg-gray-800 dark:bg-gray-700 hover:bg-gray-900 dark:hover:bg-gray-600 text-white text-sm font-semibold transition-colors rounded-xl flex items-center justify-center gap-2 shadow-sm">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button onClick={handleCheckout} className="py-2.5 bg-gradient-to-r from-[#ec3137] to-[#8a0f12] hover:from-[#8a0f12] hover:to-[#ec3137] text-white text-sm font-bold transition-all rounded-xl flex items-center justify-center gap-2 shadow-md hover:shadow-lg">
